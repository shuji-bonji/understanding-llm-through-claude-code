🌐 [English](../../06-tool-context/index.md)
...
> MCP は Claude Code における代表例でもあり、外部ツール接続のオープンな仕組みでもある。ツール定義が Context の固定費になる、という点は製品を問わない。
...
> [!TIP]
> MCP は **外部世界の事実**（API、DB、Wiki）に LLM を接地する。**コード世界の事実**（リポジトリ内のシンボル・型・参照）については [Part 9: Code Intelligence](../09-code-intelligence/index.md) を参照。両者で「事実への接地」の二本柱を構成する。
