...
同時に与える指示が増えるほど、個々の指示の遵守率は落ちる。劣化のカーブはモデルによって違う。指示の密度に上限があること自体は共通する。

他の環境での現れ方の例:

- システムプロンプトやカスタム指示にルールを詰め込むと、重要なルールも些細なルールも同程度に無視される
- レビュー観点を一度に十個並べると、半分以上が見落とされる
- 効くのは「全部書く」ことではない。同時に効いている指示の数を抑えることである
