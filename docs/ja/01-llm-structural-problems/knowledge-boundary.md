...
| **Code Intelligence（LSP）**   | 言語サーバー経由で、プロジェクト固有・カットオフ後のシンボルを解決 | [Part 9](../09-code-intelligence/index.md) を参照 — 境界が「モデルが暗記したもの」から「LSP が解決できるもの」へ移る |
...
LLM は次トークン予測で訓練されており、「知らない」への報酬が弱い。知識の外でも高い確信度で答える。カットオフ後の API や、社内固有のコードで特に顕著である。

他の環境での現れ方の例:

- 新しいフレームワークの書き方を、古い知識のまま自信を持って出す
- 社内ツールの存在しないメソッドを、型定義つきで生成する
- 公式ドキュメント、言語サーバー、テストへ接地すると、内部知識への依存が減る
