🌐 [English](../../07-runtime-layer/index.md)

# Part 7: LLMが見ないレイヤー — Settings & Hooks

> [!NOTE]
> コンテキスト外の制御である。コンテキスト予算を**一切消費しない**。
> LLM の判断に依存しない検証手段をここに置く。
> settings.json と Hooks は Claude Code における代表例である。機械的な検証をモデルの外に置く、という考え方は製品を問わない。
