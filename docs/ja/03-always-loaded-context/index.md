...
> [!NOTE]
> セッション開始時に常に読み込まれる情報。
> コンテキスト予算における「固定費」であり、最も慎重な管理が必要である。
> CLAUDE.md は Claude Code における代表例である。常駐指示を短く保つ、という考え方は製品を問わない。

## なぜ重要なのか

CLAUDE.md は LLM が**毎ターン読む**情報である。ここに書いたことは、常にコンテキスト予算を消費し続ける。書きすぎれば Priority Saturation を引き起こす。書かなすぎれば、重要な制約を知らずにコードを生成する。
