🌐 [English](../../appendix/problem-countermeasure-map.md)

# 構造的問題 × Claude Code 対策マップ（詳細版）

> [!NOTE]
> 8つの構造的問題と、Claude Code における対策の対応である。
> 対策列は代表例である。同じ粒度の機能が他ツールに揃っているとは限らない。
...
| Code Intelligence（LSP） | コード世界      | ファイル全文を読まずシンボルだけを精密に取得     |
| Agent Teams              | マルチセッション | 並列分割でどのセッションも大きく育たない         |
...
| Agent Teams           | マルチセッション     | 各セッション履歴が短く「真ん中」が形成されない |
...
| Agent Teams         | マルチセッション     | 責務をセッション間で分割し各セッションの指示数を削減 |
...
| Code Intelligence（LSP） | コード世界       | コードコミット前のシンボルレベル接地               |
...
| Code Intelligence（LSP）    | コード世界       | プロジェクト固有・カットオフ後のシンボルを言語サーバー経由で解決 |
...
| Plugins / Marketplaces | 配布           | 検証済みプロンプトを配布し、各エンジニアがゼロから試行錯誤しないようにする |
...
| Plugins / Marketplaces | 配布     | 組織の慣習を強制し、人の入れ替わりやオンボーディングを跨いで生存させる |
